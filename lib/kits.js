...
export function computeSavingsProjection({ consumo, tipo, kit }) {
  const tarifa = TARIFAS[tipo];
  const tarifaPromedio = (tarifa.min + tarifa.max) / 2;
  ...
  return {
    ...
    monthlyRange: {
      min: Math.round(consumo * tarifa.min),
      max: Math.round(consumo * tarifa.max),
    },
    yearlyData,
    roiYears,
